    ...
    public class SunGod : ModNPC
    {
        ...
        public override void AI()
        {
            if (Main.netMode != 1) {
                ...
                if (!changingPhase) AttackControl();
                npc.netUpdate = true;
            }
        }
        ...
        private float PhaseControl()
        {
            if (npc.life <= npc.lifeMax * (2f / 3f)) {
                if (PhaseCooldown < 200f)
                    ChangePhase(1);
                else if (npc.life <= npc.lifeMax * (1f / 3f))
                    if (PhaseCooldown < 400f)
                        ChangePhase(2);
            } else {
                BossPhase = 0;
            }
        ...
        private void AttackControl()
        {
            if (AttackSelection == 0f) {
                ...
                switch ((int)BossPhase) {
                    case 0:
                        if (randomF <= 1.0f) AttackSelection = 1;
        ...
        private void ChangePhase(int phase)
        {
            ...
            if (Main.netMode != 1) {
                ...
                MoveToTarget(
                    new Vector2(
                        npc.Center.X + 75f * Main.rand.NextFloat(-1f, 1f),
                        npc.Center.Y + 25f * Main.rand.NextFloat(-1f, 1f)
                    ),
                    new Vector2(0, 0),
                    16f
                );
        ...
        private void ShootFlames()
        {
            if (Main.netMode != 1)
                if (AttackCooldown == 75f) {
                    ...
                    var   velocity  = player.Center - npc.Center;
                    ...
                    var flameOffset = new Vector2(0, 0) {
                        X = 1f / normalMagn, Y = ratio / normalMagn
                    };

                    for (int i = -50; i <= 50; i += 10) {
                        float velocityRandomFactor = Main.rand.NextFloat(0.75f, 1.25f);
                        var   offsetVelocity       = velocity * velocityRandomFactor;
                        //for (int j = 0; j <= 5; ++j)
                        {
                            var temp = new Vector2(flameOffset.X, flameOffset.Y);
                            temp *= i;

                            int damage                  = (npc.damage - 30) / 2;
                            if (Main.expertMode) damage = (int)(damage / Main.expertDamage);
                            Projectile.NewProjectile(
                                new Vector2(npc.Center.X + temp.X, npc.Center.Y + temp.Y),
                                offsetVelocity,
                                type,
                                damage,
                                0f
        ...
        private void LungeAttack()
        {
            ...
            if (AttackCooldown < 25f) MoveToTarget(player.Center, new Vector2(0, -200f), 50f);
            npc.netUpdate = true;
        }
        ...
        private void NapalmRun()
        {
            ...
            if (AttackCooldown < 50f) MoveToTarget(player.Center, new Vector2(0, -200f), 25f);
            npc.netUpdate = true;
        }
        ...
        private void RingFlames(float size, int segments)
        {
            ...
            for (int i = 0; i <= segments; ++i) {
                ...
                Projectile.NewProjectile(
                    new Vector2(npc.Center.X + temp.X, npc.Center.Y + temp.Y),
                    offsetVelocity,
                    mod.ProjectileType("SunGodFlames"),
                    damage,
                    0f
                );
                velocity = velocity.RotatedBy(
                    2f * Math.PI / segments
                ); // divide the circle into segments so each value of i is equally spaced
            }
        }

        public override void OnHitPlayer(Player player, int damage, bool crit)
        {
            if (Main.expertMode) player.AddBuff(BuffID.OnFire, 600);
        }
        ...
        public override void NPCLoot()
        {
            if (Main.expertMode) {
                ...
                int[] drops = {
                    mod.ItemType("GoldenGun"), mod.ItemType("SunPowerSeed"), mod.ItemType("BeskarBar")
                };
                var dropChooser = new WeightedRandom<int>();
                for (int i = 0; i < drops.Length; ++i) dropChooser.Add(drops[i]);
                int choice = dropChooser;
                player.QuickSpawnItem(choice);
                dropChooser.Clear();
                for (int i = 0; i < drops.Length; ++i)
                    if (drops[i] != choice)
                        dropChooser.Add(drops[i]);
            ...
            if (!RAWorld.downedSunGod) {
                RAWorld.downedSunGod = true;
        ...
        {
            Main.NewText(
                "The underworld marvels at the creation of Beskar Ore",
                200,
                200,
                55
            ...
            for (int k = 0; k < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 33E-05); k++) {
                ...
                if (tile.active() && tile.type == TileID.Ash)
                    WorldGen.TileRunner(
                        ...
                        WorldGen.genRand.Next(2, 7),
                        mod.TileType("BeskarOre")
                    );
        ...
        private void DrawFace(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            if (!changingPhase)
                ...
                face = mod.GetTexture("NPCs/Bosses/SunGod/Face/SunGodCrossEyes");
